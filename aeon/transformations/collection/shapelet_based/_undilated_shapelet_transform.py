"""Undilated Shapelet transformers.
...
__all__ = ["RandomUndilatedShapeletTransform"]
...
class RandomUndilatedShapeletTransform(BaseCollectionTransformer):
    """Random  Shapelet Transform (RDST) as described in [1]_, [2]_.
    ...
    This implementation has since been modified to disable dilation, this has been done in order to benchmark performance
    and evaluate the effect dilation has on accuracy of results.
...
def _init_random_shapelet_params(
    ...
    #Dilation Randomisation (disabled)
    #====================================================#
    ...
    #=====================================================#
    #Modification to allow for no dilations

    for i in prange(max_shapelets):
        dilations[i] = np.int_(1)
