...
(RDST-RF-7.5K) with attribute selection
...
to select the top 7500 features
Refactors fit logic into a shared `_fit_rdst_shared` helper, with separate
  `_fit`, `_fit_predict`, and `_fit_predict_proba` to allow for train file generation


 Original docstring and parameter documentation retained from aeon.
"""

__all__ = ["RDSTClassifier_rotation_pipeline_2"]
...
class RDSTClassifier_rotation_pipeline_2(BaseClassifier):
    """
    ...
    def _fit_rdst_shared(self, X, y):
        ...
        self._selector = SelectKBest(score_func=mutual_info_classif, k=min(7500, X_t.shape[1]))
