...
(RDST-RF-PCA-95) with pca for dimensionality reduction to 95% explained variance.
...
the shapelet transform and before the classifier, to reduce dimensionality to 95% explained variance.


 Original docstring and parameter documentation retained from aeon.
"""

__all__ = ["RDSTClassifier_rotation_pipeline_pca_95"]
...
class RDSTClassifier_rotation_pipeline_pca_95(BaseClassifier):
    """
    A random dilated shapelet transform (RDST) classifier with a 
    Rotation Forest classifier downstream, and prncipal component 
    analysis (PCA) for dimensionality reduction to 95% explained variance.
